    ...
    "parameters": {
      ...
      "properties": {
        "responseMessage": {
          "type": "string",
          "description":
              "The response summary message from chatGPT. Answers only with 'pong' word.",
        },
      },
...
const listDirectoryToolParameters = {
  ...
  "properties": {
    "path": {
      "type": "string",
      "description":
          "Absolute or relative path to the directory to list. Use '.' for current directory",
    },
    "recursive": {
      "type": "boolean",
      "description":
          "Whether to list files recursively in subdirectories. Default is false",
    },
    "glob": {
      "type": "string",
      "description":
    ...
    "entries": {
      ...
      "description":
          "Return only files, only directories, or both. Default is all",
    },
    "exclude": {
      "type": "array",
      "items": {"type": "string"},
...
const searchFilesToolParameters = {
  ...
  "properties": {
    "pattern": {
      "type": "string",
      "description":
          "Filename pattern to search for (e.g., '*.dart', 'README.md', 'test_*'). Uses * and ? wildcards only",
    },
    "directory": {
      "type": "string",
    ...
    "skipCommonIgnored": {
      "type": "boolean",
      "description":
          "If true (default), skip common heavy directories (.git, node_modules, build, etc.) while walking the tree",
    },
  },
...
const grepToolParameters = {
  ...
  "properties": {
    ...
    "path": {
      "type": "string",
      "description":
          "File or directory to search under. Use '.' for current directory. Default '.'",
    },
    "glob": {
      "type": "string",
      "description":
          "Only search files whose path matches this glob (e.g. '*.dart'). Passed to ripgrep when available",
    },
    "max_results": {
      "type": "integer",
      "description":
          "Maximum number of matching lines (output lines) to return. Default 80",
    },
    "context_lines": {
      "type": "integer",
      "description":
          "Lines of context before/after each match when using ripgrep. Default 2; use 0 for matches only",
    },
    "case_sensitive": {
      "type": "boolean",
    ...
    "skipCommonIgnored": {
      "type": "boolean",
      "description":
          "Dart fallback only: skip .git, node_modules, build, etc. Default true",
    },
  },
  "required": ["pattern"],
};
...
const writeFileToolParameters = {
  ...
  "properties": {
    ...
    "append": {
      "type": "boolean",
      "description":
          "If true, append to file instead of overwriting. Default is false",
    },
  },
...
const executeShellCommandToolParameters = {
  ...
  "properties": {
    "command": {
      "type": "string",
      "description":
          "The shell command to execute (e.g., 'ls -la', 'git status', 'dir')",
    },
    "workingDirectory": {
      "type": "string",
      "description":
          "Optional working directory to execute command in. Default is current directory",
    },
  },
  "required": ["command"],
};

/// Agent-only stub: schedules OpenRouter server-side web search on the **next** assistant API turn.
const enableOpenRouterWebSearchToolParameters = {
  "type": "object",
  "properties": {
    "reason": {
      "type": "string",
      "description":
          "Why fresh web data is needed (news, prices, docs after knowledge cutoff, etc.)",
    },
    "query": {
      "type": "string",
      "description":
          "Suggested search query or keywords for the next turn (optional)",
    },
  },
};
...
const generateImageParameters = {
  ...
  "properties": {
    ...
    "size": {
      "type": "string",
      "description":
          "The size of the image. E.g '1024x1024'. Ensure values are less than 1440. default is 1024x1024",
    },
  },
  "required": ["prompt"],
};
